...
func SetupRouter(cfg *config.Config) *gin.Engine {
	...
	financialService := services.NewFinancialService(database.DB) // Pass DB for financial data access
	...
	refundService := services.NewRefundService(database.DB, gatewaysRegistry, refundStateMachine, refundCalculator, refundQueueService, emailOutboxService) // ✅ ADDED: emailOutboxService parameter
	...
	paymentHandler := handlers.NewPaymentHandler(paymentService, refundService, ticketService, unifiedPurchaseOrchestrator, cfg)
	...
	v1 := router.Group("/api/v1")
	{
		...
		admin.Use(middleware.IsAdminOrSubAdmin()) // Broad: admin/subadmin can access admin area
		{
			...
			adminPayments.Use(middleware.RequirePermission("read:financial"))
			{
				...
				adminPayments.POST("/refunds/:refund_id/retry", middleware.RequirePermission("create:refund"), paymentHandler.AdminRetryRefund)     // ✅ FIXED: Changed from update:refund to create:refund
		...
		// Organizer routes - approved organizer access (broad access control)
